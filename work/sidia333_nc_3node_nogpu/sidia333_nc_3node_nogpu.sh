...
#PBS -N s333_3node_nogpu
#PBS -j o
#PBS -o sidia333_nc_3node_nogpu.joblog

# OpenMX 4.0 GPU: 216 Si, s2p2d1, non-collinear -- every matrix is a
# 2-component spinor, so the dimension is 5616 rather than the 2808 of the
# collinear ../sidia333_* series.  Band solver, 2x2x2 k-grid, 25 SCF.  3 Pegasus node(s), 48 ranks
...
# an earlier 1 node / 48 ranks run with gpusolver+MPS: Utot =
# -887.422830856493, total 250.3 s, DFT 238.5 s, Diagonalization 162.9 s.
# That point is re-measured here so the whole series comes from one script
# and one binary.
...
echo "=== Utot (earlier 1-node gpusolver run: -887.422830856493) ==="
grep -E "Utot\." "${CASE}.out" 2>/dev/null | tail -2
echo "=== timing (earlier 1-node gpusolver: total 250.3 / DFT 238.5 / Diag 162.9 s) ==="
