#!/bin/bash
# Aggregate the 24 sidia444_nc_cluster repetitions (2/3/4/6 nodes x GPU/CPU x 3,
...
#
# Run from work/:  ./summarize_s444nc_reps.sh
set -u
cd "$(dirname "$0")" || exit 1
...
echo "sidia444_nc_cluster node scaling, 48 ranks/node, 3 repetitions per point"
...
for n in 2 3 4 6; do
  collect "s444nc_${n}n_g1 s444nc_${n}n_g2 s444nc_${n}n_g3 s444nc_${n}n_c1 s444nc_${n}n_c2 s444nc_${n}n_c3" | \
...
: > /tmp/.s444nc_means
for n in 2 3 4 6; do
  collect "s444nc_${n}n_g1 s444nc_${n}n_g2 s444nc_${n}n_g3" | \
    stats "${n} node(s), GPU+MPS" gpu "$n" 2>>/tmp/.s444nc_means
  collect "s444nc_${n}n_c1 s444nc_${n}n_c2 s444nc_${n}n_c3" | \
    stats "${n} node(s), CPU elpa2" cpu "$n" 2>>/tmp/.s444nc_means
done
echo
...
rm -f /tmp/.s444nc_means
echo

echo "--- Utot (within a (path, node count): identical; across: ~12 digits) ---"
for n in 2 3 4 6; do
  for c in s444nc_${n}n_g1 s444nc_${n}n_g2 s444nc_${n}n_g3 s444nc_${n}n_c1 s444nc_${n}n_c2 s444nc_${n}n_c3; do
...
echo "--- peak host memory, node 0 (GiB); old refs: GPU 2n 96(timeout) 3n OOM@119 4n 97, CPU 2n 90 ---"
for n in 2 3 4 6; do
  for r in 1 2 3; do
    c=s444nc_${n}n_g${r}
