...
#PBS -N rtb_s_gpu
#PBS -j o
#PBS -o rtb_s_gpu.joblog

# README benchmark: OpenMX "-runtest" (14 small systems), GPU run with the
# input-file defaults (scf.eigen.lib gpusolver, scf.gemmul8.enable on --
# i.e. GEMMul8 INT8-based FP64 GEMM emulation active).  1 Pegasus gen_S node
# (Xeon Platinum 8468, 48 cores + 1 H100 PCIe), flat MPI 48 ranks, -nt 1,
# all ranks sharing the GPU through CUDA MPS (the production configuration
# on this machine).  Same binary as the CPU runs (work/openmx, v2.0_thesis
# tag build; identity recorded in the per-case *.manifest.json).
# input_example/ and ../DFT_DATA19 are reached through symlinks so the four
