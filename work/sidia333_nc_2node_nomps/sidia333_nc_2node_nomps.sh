...
#PBS -N sd333nc_2n_nomps
#PBS -j o
#PBS -o sidia333_nc_2node_nomps.joblog

# OpenMX 4.0 GPU: 216 Si, s2p2d1, non-collinear (dimension 5616), Band solver,
# 2x2x2 k-grid, 25 SCF iters.  Two Pegasus nodes, 48 ranks each = 96 ranks, one
# H100 per node, **NVIDIA MPS deliberately NOT enabled**.
#
# This is the no-MPS control for ../sidia333_nc_2node (same case, same 96 ranks,
# MPS on both nodes):
#
#   ../sidia333_nc_2node (MPS on) : DFT 146.9 s, Diagonalization 95.6 s,
#                                   Set_Hamiltonian 10.6 s, Utot -887.422830856498
#
# Utot must not move; only the timings should.  It is also the control for the
# teardown hang seen on that run, where openmx printed its completion banner at
# 13:03:08 and mpirun then sat for 13 more minutes with an empty joblog and no
# stderr.  If the hang recurs here with MPS off, MPS was not the cause.
...

{
  ...
  echo "=== workdir   : $(pwd)"
  echo "=== layout    : ${NNODE} nodes x ${PPN} ranks = ${NPROC} ranks, MPS OFF"
  echo "=== mpirun    : $(command -v mpirun)"
  echo "=== NQSV_MPIOPTS = [${NQSV_MPIOPTS:-}]"
  echo "=== nodes ==="
  mpirun ${NQSV_MPIOPTS:-} -np "${NNODE}" -npernode 1 hostname
  echo "=== MPS control daemons per node (expect 0 on each) ==="
...
echo "=== Utot (2-node MPS reference: -887.422830856498) ==="
grep -E "Utot\." "${CASE}.out" 2>/dev/null | tail -2
echo "=== timing (2-node MPS reference: DFT 146.9 / Diag 95.6 / SetH 10.6 s) ==="
