...
#PBS -N cghelgeo_ppn36
#PBS -j o
#PBS -o cghelgeo_ppn36.joblog

# OpenMX 4.0 GPU: cghelgeo_ppn36.dat -- 650 atoms over 5 species (190 C, 240 H, 80 N,
...
# Two Pegasus nodes, 36 ranks per node = 72 ranks, MPS on both nodes.
#
# WHY 36 RANKS PER NODE
# ---------------------
# Third point on a measured memory-vs-ranks curve for this case, after
# ../cghelgeo at 12/node and ../cghelgeo_ppn24 at 24/node.  The 12/node run
# settled at a steady 68 GiB of the node's 124 GiB (GPU peak 50 GB, 100% util),
# so the extrapolation that said 12/node would not even fit was wrong by a wide
# margin, and guessing again is pointless -- hence measuring three points.
#
# The reason more ranks need not cost more memory here: with only 3 k-points and
# 72 ranks, OpenMX splits each k-point across a large process group, so each
# rank's share of the 10430^2 matrices shrinks as ranks grow.  Pulling the other
# way is per-rank grid and atom overhead, which does grow.  36/node is where the
# second term should start to dominate if it ever does.
#
# .smi samples host memory every 10 s, so an OOM here still yields the number
# that says how far over the node it went.
...
PPN=36
NPROC=$((NNODE * PPN))
STDOUT=cghelgeo_ppn36.std
STDERR=cghelgeo_ppn36.err
ENVLOG=cghelgeo_ppn36.env
SMILOG=cghelgeo_ppn36.smi
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 3000 \
       ${MPS_X} \
       ../openmx cghelgeo_ppn36.dat -nt 1 > "$STDOUT" 2> "$STDERR" &
MPIPID=$!
...
grep -E "Utot\." cghelgeo_ppn36.out 2>/dev/null | tail -2
