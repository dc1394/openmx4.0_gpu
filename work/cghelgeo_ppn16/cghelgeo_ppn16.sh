...
#PBS -N cghelgeo_ppn16
#PBS -j o
#PBS -o cghelgeo_ppn16.joblog

# OpenMX 4.0 GPU: cghelgeo_ppn16.dat -- 650 atoms over 5 species (190 C, 240 H, 80 N,
...
# Two Pegasus nodes, 16 ranks per node = 32 ranks, MPS on both nodes.
#
# WHY 16 RANKS PER NODE
# ---------------------
# The binding resource for this case is GPU memory, not host memory -- the
# opposite of sidia444_nc, and the opposite of what the rank count was first
# chosen for.  Measured on this exact input:
#
#   12/node : host 68/124 GiB, GPU 48.8/79.2 GiB, 100% util -- ran fine
#   24/node : GPU OOM in the VNA projector, before SCF even started
#   36/node : same GPU OOM
#
#   "Out of memory allocating 3137863680 bytes of device memory
#    total/free CUDA memory: 85017624576/316997632"
#
# At 24/node the host still had 107 GiB free when the H100 ran dry, so host
# memory was never the limit at these rank counts.
#
# 48.8 GiB over 12 ranks is ~4.07 GiB/rank, putting the ceiling at 79.2/4.07 =
# ~19 ranks/node, which is exactly why 24 failed.  16/node should land near
# 65 GiB, and it stays under even on the pessimistic reading of the two data
# points (treating all 48.8 GiB as per-rank rather than partly fixed).
#
# .smi samples the GPU every 10 s, so this either confirms the 4.07 GiB/rank
# model or gives the correction.
...
PPN=16
NPROC=$((NNODE * PPN))
STDOUT=cghelgeo_ppn16.std
STDERR=cghelgeo_ppn16.err
ENVLOG=cghelgeo_ppn16.env
SMILOG=cghelgeo_ppn16.smi
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 27000 \
       ${MPS_X} \
       ../openmx cghelgeo_ppn16.dat -nt 1 > "$STDOUT" 2> "$STDERR" &
MPIPID=$!
...
grep -E "Utot\." cghelgeo_ppn16.out 2>/dev/null | tail -2
