...
#PBS -j o
#PBS -o sidia444_cluster_nogpu_1node.joblog
...
#
#  1 Pegasus node(s), 48 ranks
# each = 48 ranks.  elpa2 on the CPU: no GPU requested, no MPS.
#
...
PPN=48
NPROC=$((NNODE * PPN))
CASE=sidia444_cluster_nogpu_1node
STDOUT=${CASE}.std
STDERR=${CASE}.err
ENVLOG=${CASE}.env
MEMLOG=${CASE}.smi
