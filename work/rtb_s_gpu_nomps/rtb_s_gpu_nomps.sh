...
#PBS -N rtb_s_gnom
#PBS -j o
#PBS -o rtb_s_gpu_nomps.joblog

# README benchmark: OpenMX "-runtest" (14 small systems), GPU run WITHOUT
# CUDA MPS -- the 48 ranks time-slice the H100 with one CUDA context each.
# This is the "H100 GPU, MPS off" column of the README table; everything
# else is identical to rtb_s_gpu (input-file defaults: scf.eigen.lib
# gpusolver, scf.gemmul8.enable on).  1 Pegasus gen_S node (Xeon Platinum
# 8468, 48 cores + 1 H100 PCIe), flat MPI 48 ranks, -nt 1.  Same binary as
# every other column (work/openmx, v2.0_thesis tag build; identity recorded
# in the per-case *.manifest.json).  input_example/ and ../DFT_DATA19 are
# reached through symlinks so the benchmark configs run isolated.

set -u
ulimit -c 0
NVROOT=/system/apps/ubuntu/22.04-202604/nvhpc/26.5/Linux_x86_64/26.5
CASE=rtb_s_gpu_nomps
