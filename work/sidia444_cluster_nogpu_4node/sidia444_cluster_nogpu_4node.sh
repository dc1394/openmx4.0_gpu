...
#PBS -j o
#PBS -o sidia444_cluster_nogpu_4node.joblog

# OpenMX 4.0 GPU: 512 Si, s2p2d1 -> 6656 basis functions.  Cluster solver at
# the gamma point only (scf.Kgrid 1 1 1) and collinear spin, so the matrices are
# real symmetric 6656x6656 -- 0.35 GB each, against the 2.84 GB complex matrices
# of the Band/non-collinear sidia444_nc at dimension 13312.  25 SCF iterations
# to 1e-13, energycutoff 200 Ry.#
# Host memory is worth watching rather than assuming here: the 512-atom grid at
# 200 Ry put a ~63 GiB floor under sidia444_nc, and at 48 ranks on one node that
# whole floor sits on one 124 GiB host.  The matrices are far lighter than that
# case, so it should fit, but .smi samples host memory every 10 s and will show
# exactly where it lands.
#
#  4 Pegasus node(s), 48 ranks
# each = 192 ranks.  elpa2 on the CPU: no GPU requested, no MPS.
#
...
PPN=48
NPROC=$((NNODE * PPN))
CASE=sidia444_cluster_nogpu_4node
STDOUT=${CASE}.std
STDERR=${CASE}.err
ENVLOG=${CASE}.env
MEMLOG=${CASE}.smi
