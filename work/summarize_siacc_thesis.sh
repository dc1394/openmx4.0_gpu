#!/bin/bash
# THESIS (v2.0_thesis tag) accuracy campaign aggregate (plan v2.6 sec. 7.5 / 13.2-B / 13.3):
#   siacc_{bcol,bnc,ccol,cnc}_{c,o,g}11 (+ bnc o12/g12)  displaced Si 216
...
# Run from work/:  ./summarize_siacc_thesis.sh
set -u
cd "$(dirname "$0")" || exit 1
...
for c in siacc_bnc_o12 siacc_bnc_g12; do
  printf "%-16s %6s %-24s %-14s %-12s\n" "$c" "$(nscf "$c")" "$(utot "$c")" "$(chemp "$c")" "$(fmax "$c")"
done
echo
...
for s in bcol bnc ccol cnc; do
  printf "  %-6s %-14s %-14s %-14s %-14s\n" "$s" \
    "$(ediff siacc_${s}_c11 siacc_${s}_o11)" "$(fdiff siacc_${s}_c11 siacc_${s}_o11)" \
    "$(ediff siacc_${s}_g11 siacc_${s}_o11)" "$(fdiff siacc_${s}_g11 siacc_${s}_o11)"
done
echo
echo "--- run-to-run baseline (bnc, same config twice) ---"
printf "  cuBLAS o11-o12:  dE=%s  dFmax=%s\n" "$(ediff siacc_bnc_o11 siacc_bnc_o12)" "$(fdiff siacc_bnc_o11 siacc_bnc_o12)"
printf "  GEMMul8 g11-g12: dE=%s  dFmax=%s\n" "$(ediff siacc_bnc_g11 siacc_bnc_g12)" "$(fdiff siacc_bnc_g11 siacc_bnc_g12)"
echo
...
printf "  dE(CPU-cuB)=%s dFmax=%s | dE(G8-cuB)=%s dFmax=%s\n" \
  "$(ediff siacc_mno_c11 siacc_mno_o11)" "$(fdiff siacc_mno_c11 siacc_mno_o11)" \
  "$(ediff siacc_mno_g11 siacc_mno_o11)" "$(fdiff siacc_mno_g11 siacc_mno_o11)"
...
for c in siacc_bcol_o11 siacc_bnc_o11 siacc_ccol_o11 siacc_cnc_o11 siacc_mno_o11 siacc_cr2_o11 siacc_gaas_o11; do
