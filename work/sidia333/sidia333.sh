...
#PBS --cpunum-lhost=48
#PBS --gpunum-lhost=1
# The same case ran in 105 s wall (DFT 67 s, of which Diagonalization 40 s), so
# an hour is ~30x margin and schedules sooner than a 6 h request.
#PBS -l elapstim_req=01:00:00
#PBS -T openmpi
#PBS -v NQSV_MPI_VER=nvhpc-hpcx-cuda13/25.11
#PBS -N sidia333
#PBS -j o
#PBS -o sidia333.joblog

# OpenMX 4.0 GPU: sidia333.dat (216 Si, s2p2d1 -> matrix dim 2808, Band solver,
# 2x2x2 k-grid, 25 SCF iters) on one Pegasus node, flat MPI 48 ranks sharing a
# single H100 PCIe *through NVIDIA MPS*.
#
...
# .joblog, and openmx's own output) stays under work/sidia333.
...
# PBS_O_WORKDIR is work/sidia333, so ../openmx is the binary and the input's
...
mpirun ${NQSV_MPIOPTS:-} -np 48 -npernode 48 \
       ...
       ../openmx sidia333.dat -nt 1 > "$STDOUT" 2> "$STDERR"
rc=$?
