...
# Run from work/:  ./summarize_siband_scaling.sh
set -u
cd "$(dirname "$0")" || exit 1

reps() {  # $1=mode-atoms  $2=N  $3=b   -> rep numbers to pool
  # nc216 1n g1 AND g3 both drew slow node bnode033 (294.0 / 293.4 s vs
  # ~256-260 s on clean nodes): discarded per site convention, replaced by
  # g4 and g5.
  if [ "$1" = nc216 ] && [ "$2" = 1 ] && [ "$3" = g ]; then echo "2 4 5"; return; fi
  # CV>2% top-ups (plan 7.4): these three points carry 5 reps.  col300 4n g4
  # had bnode033 in its 4-node set (75.6 s, inflated): discarded, g6 instead.
  if [ "$1" = col300 ] && [ "$2" = 2 ] && [ "$3" = o ]; then echo "1 2 3 4 5"; return; fi
  if [ "$1" = col300 ] && [ "$2" = 4 ] && [ "$3" = o ]; then echo "1 2 3 4 5"; return; fi
  if [ "$1" = col300 ] && [ "$2" = 4 ] && [ "$3" = g ]; then echo "1 2 3 5 6"; return; fi
  echo "1 2 3"
}
