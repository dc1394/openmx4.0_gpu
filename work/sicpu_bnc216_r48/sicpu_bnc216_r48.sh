...
#PBS -l elapstim_req=01:10:00
#PBS -T openmpi
#PBS -v NQSV_MPI_VER=nvhpc-hpcx-cuda13/25.11
#PBS -N cpbn216_48
#PBS -j o
#PBS -o sicpu_bnc216_r48.joblog

# CPU best-config preflight (plan v2.6 sec. 6.2 / step 10): sicpu_bnc216_r48.
# 3-SCF + force, scf.eigen.lib elpa2, flat MPI: 1 node x 48 ranks (-nt 1),
# whole node reserved (--cpunum-lhost=48), no GPU.  216 Si atoms, band solver, Kgrid 2x2x2 (8 computed k).
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 3840 \
