...
#PBS -N s443_2e1
#PBS -j o
#PBS -o s443nc_2n_e1.joblog

# sidia443_nc_cluster campaign, GPU path (gpusolver2, ELPA 1-stage) -- one of 36 requests
...
export OPENMX_GS2_ELPA_SOLVER=1
MPS_X="-x CUDA_MPS_PIPE_DIRECTORY -x CUDA_MPS_LOG_DIRECTORY -x OPENMX_GS2_ELPA_SOLVER"
...

{
  ...
  echo "=== layout    : ${NNODE} nodes x ${PPN} ranks = ${NPROC} ranks, 1 H100/node, MPS on, gpusolver2"
  echo "=== nodes ==="
