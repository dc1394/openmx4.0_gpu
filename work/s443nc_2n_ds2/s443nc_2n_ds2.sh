...
#PBS -N s443_2ds2
#PBS -j o
#PBS -o s443nc_2n_ds2.joblog

# sidia443_nc_cluster campaign, GPU path (gpusolver2) -- one of 36 requests
# ({1,2,3,4} nodes x {gpusolver2, gpusolver, elpa2} x 3
# reps, all -npernode 36) so Total and Diagonalization come with a mean.
# Binary: gpusolver2-merge (commit f5fb5b3).  384 Si, s2p2d1, non-collinear -> matrix
...
export OPENMX_GS2_SOLVE_RPN=1
export OPENMX_GS2_SOLVE_DEBUG=1
MPS_X="-x CUDA_MPS_PIPE_DIRECTORY -x CUDA_MPS_LOG_DIRECTORY -x OPENMX_GS2_SOLVE_RPN -x OPENMX_GS2_SOLVE_DEBUG"
...

{
  ...
  echo "=== layout    : ${NNODE} nodes x ${PPN} ranks = ${NPROC} ranks, 1 H100/node, MPS on, gpusolver2, OPENMX_GS2_SOLVE_RPN=1"
  echo "=== nodes ==="
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 420 \
       ${MPS_X} \
