...
#PBS -N cghelgeo_ppn48
#PBS -j o
#PBS -o cghelgeo_ppn48.joblog

# OpenMX 4.0 GPU: cghelgeo_ppn48.dat -- 650 atoms over 5 species (190 C, 240 H, 80 N,
...
#
# WHY 48 RANKS PER NODE
# ---------------------
# Last point of the ppn scan for this case: 12, 24, 36, now 48 (a full node).
# Measured so far, all 2 nodes, MPS on, 70 SCF iterations to 1e-13:
#
#   ppn  total   host peak   GPU peak   DS_VNA     wall
#    12    24    68 GiB      48.8 GiB   on GPU     (killed at SCF 48/70)
#    24    48    83 GiB      54.3 GiB   CPU        485.2 s
#    36    72    92 GiB      59.7 GiB   CPU        505.9 s
#
# 24 is the best so far and 36 is already 4% slower: the single H100 fixes the
# diagonalisation throughput, so extra ranks only add ScaLAPACK split and
# communication overhead (Diagonalization went 376.6 -> 400.9 s).  48/node is
# expected to be slower still; it is run to confirm the turn is monotonic rather
# than assumed, and to see whether host memory -- 68, 83, 92 GiB so far, against
# 124 -- finally becomes the binding constraint now that DS_VNA runs on the CPU.
#
# The DS_VNA device estimate is 3.274 GiB/rank and does not depend on rank count,
# so 48 ranks would ask 157 GiB of an 80 GiB card: the CPU fallback added in
# Set_ProExpn_VNA.c will take that path, as it did at 24 and 36.
...
PPN=48
NPROC=$((NNODE * PPN))
STDOUT=cghelgeo_ppn48.std
STDERR=cghelgeo_ppn48.err
ENVLOG=cghelgeo_ppn48.env
SMILOG=cghelgeo_ppn48.smi
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 3000 \
       ${MPS_X} \
       ../openmx cghelgeo_ppn48.dat -nt 1 > "$STDOUT" 2> "$STDERR" &
MPIPID=$!
...
grep -E "Utot\." cghelgeo_ppn48.out 2>/dev/null | tail -2
