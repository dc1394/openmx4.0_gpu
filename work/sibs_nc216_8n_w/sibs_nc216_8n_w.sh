...
#PBS -N sn216_8w
#PBS -j o
#PBS -o sibs_nc216_8n_w.joblog

# Si diamond BAND limited strong scaling (plan v2.6 sec. 7.4/8.5): sibs_nc216_8n_w.
# 8 node(s) x 48 ranks (-nt 1), 1 H100/node, MPS on; same input at every
# node count (strong scaling, k-point parallelism over the 8 computed
# k points: 8/4/2/1 k per GPU at 1/2/4/8 nodes).  Config: 3-SCF preflight: warm-up + path check, NOT pooled into means.
# System: 216 Si atoms from the sec.-7.3 size search (nc band representative);
# deck copied from the confirmed sib_nc216 case, with scf.criterion 1e-15 so
# every run does exactly 3 SCF iterations (1e-13 converged at 24 once).
...

{
  ...
  echo "=== layout    : ${NNODE} nodes x ${PPN} ranks = ${NPROC} ranks, 1 H100/node, MPS on, gemmul8 on"
  echo "=== nodes ==="
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 1440 \
       ${MPS_X} \
