...
#PBS -N s443nc36g
#PBS -j o
#PBS -o s443nc_1n36_g.joblog

# GO/NO-GO probe: sidia443_nc_cluster (384 Si, s2p2d1, non-collinear ->
# matrix dimension 9984, 1.60 GiB per dense complex matrix, Cluster solver,
# 25 SCF iterations), ONE node x 48 ranks, GPU path (gpusolver + MPS),
# round-3 binary (commit e227507).
#
# First run of this input ever -- no Utot reference.  The question is whether
# one H100 takes 48 NC ranks: sidia444_nc (512 atoms, dim 13312) failed at
# 1 node in every layout tried (np48/np24/np12), killed by device capacity ->
# cascading CPU fallbacks -> host OOM.  This system is 3/4 the atoms and
# 0.56x the dense-matrix footprint, so it may fit where 444 could not.
...
# the result: this probe is specifically "48 NC ranks under MPS on one H100".
...
# Sampler for THIS node only, every 5 s: a 1-node host OOM dies within the
# first minutes, so 10 s sampling would miss the climb.  "trap -" so a kill
# actually kills it.
( trap - EXIT INT TERM
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 5400 \
       ${MPS_X} \
...
echo "=== Utot (first run of this input; no reference) ==="
