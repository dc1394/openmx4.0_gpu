#!/bin/bash
# RTX 5080 local benchmark (rtx5080_procedure.md; plan v2.6 sec. 7.2/8.2/8.3): s5a_bcol_g11.
...
# Convergence run (up to 60 SCF, criterion 1e-13); displaced Si:
# atom 1 moved +0.05 Ang along x so forces are non-zero (plan 7.5-1). MD.Type Opt / MD.maxIter 1; wall time = Max_Time column
...

{
  echo "=== host      : $(hostname)"
  echo "=== date      : $(date)"
  echo "=== layout    : 1 node x ${NPROC} ranks, 1 RTX 5080, MPS on, gemmul8 on"
