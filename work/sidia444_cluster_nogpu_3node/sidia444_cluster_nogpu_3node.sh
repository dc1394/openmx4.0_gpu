...
#PBS -j o
#PBS -o sidia444_cluster_nogpu_3node.joblog
...
#
#  3 Pegasus node(s), 48 ranks
# each = 144 ranks.  elpa2 on the CPU: no GPU requested, no MPS.
#
...
PPN=48
NPROC=$((NNODE * PPN))
CASE=sidia444_cluster_nogpu_3node
STDOUT=${CASE}.std
STDERR=${CASE}.err
ENVLOG=${CASE}.env
MEMLOG=${CASE}.smi
