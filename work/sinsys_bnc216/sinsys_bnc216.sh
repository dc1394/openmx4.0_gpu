...
#PBS -N nsys_bnc
#PBS -j o
#PBS -o sinsys_bnc216.joblog

# Nsight Systems toolchain GO/NO-GO (plan v2.6 sec. 9.2): band NC 216,
# 3-SCF cuBLAS + MPS, 48 ranks; ranks 0-1 run under nsys (see
# nsys_wrap.sh), the rest run openmx directly.  Deliverable: .nsys-rep
# files proving CUDA timeline capture works on a Pegasus H100 node with
# MPS.  Wall time from this run is PROFILING-GRADE ONLY, never pooled.
# Binary: cd5f0d5 + GEMMul8 v3.2.0 (md5 962f8d2519c2e6aa5a6295513f76fee9).
...
echo "=== DC-LNO GPU dispatch lines ==="
grep -E "<DC-LNO>" "$STDOUT" 2>/dev/null | head -6
