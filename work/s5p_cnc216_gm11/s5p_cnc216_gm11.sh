#!/bin/bash
# RTX 5080 local benchmark (rtx5080_procedure.md; plan v2.6 sec. 7.2/8.2/8.3): s5p_cnc216_gm11.
# 216 Si atoms from sidia.dat supercells, cluster solver, Gamma-only.
# Config: GPU gpusolver, MPS OFF, gemmul8 on (plan 8.3 MPS x GEMMul8 interaction).
...

{
  echo "=== host      : $(hostname)"
  echo "=== date      : $(date)"
  echo "=== layout    : 1 node x ${NPROC} ranks, 1 RTX 5080, MPS OFF, gemmul8 on"
...
: # MPS OFF: no daemon started (plan 8.3 ablation); unique pipe dir keeps any stray daemon unreachable
...
if [ "$(grep -c 'control_pipe=no' mps_check.tmp)" -ne 1 ]; then
  echo "FATAL: expected this node with control_pipe=no, got:" | tee -a "$ENVLOG"
  cat mps_check.tmp | tee -a "$ENVLOG"
  rm -f mps_check.tmp
  exit 1
fi
rm -f mps_check.tmp
progress "MPS state verified (control_pipe=no)"
