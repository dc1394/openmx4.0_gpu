...
#PBS -N pmfb2
#PBS -j o
#PBS -o simani_fb2.joblog

# H100 1-node production benchmark (plan v2.6 sec. 8.1/8.3): simani_fb2.
# 360 Si atoms from sidia.dat supercells, cluster solver, Gamma-only.
# Config: GPU gpusolver + MPS on, gemmul8 off -> cuBLAS FP64 (Layer 1/3 baseline).
...
set -u
ulimit -c 0
export OPENMX_CLUSTER_GPU_DIAG_RESERVE_MB=999999
...
MPS_X="-x CUDA_MPS_PIPE_DIRECTORY -x CUDA_MPS_LOG_DIRECTORY -x OPENMX_CLUSTER_GPU_DIAG_RESERVE_MB"
