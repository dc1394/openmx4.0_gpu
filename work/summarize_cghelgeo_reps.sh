#!/bin/bash
# Aggregate the six cghelgeo repetitions (3 GPU + 3 CPU, 2 nodes x 48 = 96 ranks).
#
...
GPU_CASES="cghelgeo_rep1 cghelgeo_rep2 cghelgeo_rep3"
CPU_CASES="cghelgeo_nogpu_rep1 cghelgeo_nogpu_rep2 cghelgeo_nogpu_rep3"
...
echo "--- statistics ---"
echo "$gpu_rows" | stats "GPU (gpusolver+MPS)" gpu 2>/tmp/.cg_gpu_means
echo
echo "$cpu_rows" | stats "CPU (elpa2)"         cpu 2>/tmp/.cg_cpu_means
echo
...
' /tmp/.cg_gpu_means /tmp/.cg_cpu_means
rm -f /tmp/.cg_gpu_means /tmp/.cg_cpu_means
echo
