...
#PBS -N runtest_mps48
#PBS -j o
#PBS -o runtest_mps_np48.joblog

# OpenMX 4.0 GPU: "./openmx -runtest" -- the 14 acceptance cases in
# work/input_example, each run and then compared against its stored reference
# .out.  One Pegasus node, 48 ranks, one H100, NVIDIA MPS enabled.
#
# Unlike the sidia333_*/cghelgeo_* jobs this runs in work/ itself, because
# Runtest.c resolves "input_example" and writes runtest.result relative to the
# current directory.
#
# openmx.c caps -runtest at 64 MPI processes, so 48 is inside the limit.
#
# The no-MPS run of exactly this configuration took 99.50 s of summed case time
# (kept as work/runtest_nomps.result), so the pair shows what MPS is worth on
# the acceptance suite.  Note the cases are small -- dimensions 8 to 52, all
# below the 400 threshold -- so their dense eigensolves take the CPU fallback
# and only the GPU kernel paths are exercised.
...

{
  ...
  echo "=== runtest inputs ==="
  ls input_example/*.dat | wc -l
  echo "=== previous no-MPS result (for comparison) ==="
  tail -1 runtest_nomps.result 2>/dev/null
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 3000 \
       ${MPS_X} \
       ./openmx -runtest -nt 1 > "$STDOUT" 2> "$STDERR" &
MPIPID=$!
progress "mpirun launched (pid ${MPIPID})"

# Foreground watchdog, deliberately not a background subshell.  The marker here
# is NOT the usual "normally finished" banner: -runtest prints that once per
# case, so matching it would tear mpirun down after the first of 14.  Runtest.c
# emits the line below exactly once, from the host rank, after the whole suite
# has been compared -- see the tail of Runtest().
DONE_MARK="The comparison can be found in a file"
...
echo "=== cases that reported normal completion (expect 14) ==="
grep -c "The calculation was normally finished" "$STDOUT" 2>/dev/null
# Keep this run's verdict under its own name; runtest.result is rewritten by
# every -runtest invocation.
cp -f runtest.result "${CASE}.result" 2>/dev/null
echo "=== runtest.result (MPS) ==="
cat "${CASE}.result" 2>/dev/null
echo "=== same suite without MPS, for comparison ==="
tail -1 runtest_nomps.result 2>/dev/null
