...
#PBS -N accolg11
#PBS -j o
#PBS -o siacc_ccol_g11.joblog

# Accuracy campaign (plan v2.6 sec. 7.5 / 13.2-B): siacc_ccol_g11.
# Displaced Si: 216 atoms, atom 1 moved +0.05 Ang along x, so
# forces are non-zero and DISCRIMINATE the configs (plan 7.5-1).  Runs to
# convergence (criterion 1e-13, up to 60 SCF).
# Config: GPU gpusolver + MPS, gemmul8 on (INT8 FP64 emulation).  48 ranks, -nt 1, whole node.
...
echo "=== SCF iterations completed (of 60-100 max) ==="
grep -cE "SCF=" "$STDOUT" 2>/dev/null
echo "=== GPU dispatch / fallback lines ==="
grep -E "<Band_DFT_Col>|<Band_DFT_NonCol>|<Band>|<Cluster_DFT_(Col|NonCol)>|<Set_Hamiltonian> GPU device" "$STDOUT" 2>/dev/null | head -8
