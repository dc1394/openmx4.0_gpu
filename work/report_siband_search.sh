...
# Idempotent: re-run any time; it reads only sib_*/ artifacts.
...
row() {  # $1=case  -> one table line (empty if case dir absent)
  ...
  fb=$(grep -ciE "cannot fit" "$jl" 2>/dev/null)
  local used=${rss%%/*}
...
echo "======================================================================"
echo "Si diamond BAND-solver 1-node size search  (research plan v2.6, 7.3)"
echo "======================================================================"
...
echo "  [col band: n = 13*N real]"
for a in 216 250 288 300 360 384 432 512; do row "sib_col${a}_p"; done
for a in 300 360; do row "sib_col${a}_p8"; done
for a in 300 360; do row "sib_col${a}_o"; row "sib_col${a}_g"; done
echo "  [NC band: 2n = 26*N complex]"
for a in 216 250 288 300 360 384; do row "sib_nc${a}_p"; done
for a in 216 250; do row "sib_nc${a}_p8"; done
for a in 216 250; do row "sib_nc${a}_o"; row "sib_nc${a}_g"; done
echo
...
for c in sib_col300_p sib_col300_p8 sib_col360_p sib_col360_p8 sib_nc250_p sib_nc250_p8; do
  [ -s "$c/$c.joblog" ] || continue
  b=$(grep -m1 -E "<Band_DFT_(Col|NonCol)> GPU device" "$c/$c.joblog" | sed 's/^ *//')
  printf "  %-16s %s\n" "$c" "${b:-?}"
done
echo
...
for a in 216 250 288 300 360; do
  for m in col nc; do
    new="sib_${m}${a}_p/sib_${m}${a}_p.out"; old="siband_v311_superseded/sib_${m}${a}_p/sib_${m}${a}_p.out"
...
for a in 300 360; do
  o="sib_col${a}_p/sib_col${a}_p.out"; g="sib_col${a}_p8/sib_col${a}_p8.out"
  [ -s "$o" ] && [ -s "$g" ] || continue
...
for a in 216 250; do
  o="sib_nc${a}_p/sib_nc${a}_p.out"; g="sib_nc${a}_p8/sib_nc${a}_p8.out"
  [ -s "$o" ] && [ -s "$g" ] || continue
...
for d in sib_*_p sib_*_p8 sib_*_o sib_*_g; do
  [ -s "$d/$d.env" ] || continue
