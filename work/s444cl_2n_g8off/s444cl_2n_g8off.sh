...
#PBS -N s444g8off
#PBS -j o
#PBS -o s444cl_2n_g8off.joblog

# sidia444_cluster node-scaling rerun, GPU PATH -- one of 24 requests
# (4 node counts x {gpusolver+MPS, elpa2} x 3 repetitions) so that Total and
...
#
# Input: sidia444_cluster.dat with only System.Name changed.  512 Si, s2p2d1 ->
# matrix dimension 6656, real symmetric, Cluster solver at the gamma point,
# collinear, 25 SCF iterations to 1e-13, energycutoff 200 Ry.
#
# Single-sample references (old binary, Max_Time): GPU 2n 77.9 s / 3n 74.6 s /
# 4n 74.0 s.  The old 1-node GPU run OOMed on the host (>=99/124 GiB when the
# OOM killer fired) -- whether it now fits IS one of the measurements, so a
# crash here is a result, not a failure of the harness.
# Utot reference: -2103.47785856322x (last digits move with the rank count;
# ~12-digit agreement with the elpa2 value -2103.47785856333x is expected).
...
mpirun ${NQSV_MPIOPTS:-} -np "${NPROC}" -npernode "${PPN}" --timeout 1500 \
       ${MPS_X} \
...
echo "=== Utot (reference: -2103.47785856322x, elpa2 gives ...333x) ==="
grep -E "Utot\." "${CASE}.out" 2>/dev/null | tail -2
echo "=== SCF iterations completed (of 25 max) ==="
grep -cE "SCF=" "$STDOUT" 2>/dev/null
echo "=== peak host memory, this node (GiB); old 1-node GPU died at >=99/124 ==="
