...
#PBS -N pbn216om12
#PBS -j o
#PBS -o sip_bnc216_om12.joblog

# H100 1-node production benchmark (plan v2.6 sec. 8.1/8.3): sip_bnc216_om12.
...

{
  ...
  echo "=== layout    : ${NNODE} nodes x ${PPN} ranks = ${NPROC} ranks, 1 H100/node, MPS OFF, gemmul8 off"
  echo "=== nodes ==="
...
: # MPS OFF: no daemon started (plan 8.3 ablation)
...
if [ "$(grep -c 'control_pipe=no' mps_check.tmp)" -ne "${NNODE}" ]; then
  echo "FATAL: expected ${NNODE} nodes WITHOUT MPS (ablation), got:" | tee -a "$ENVLOG"
