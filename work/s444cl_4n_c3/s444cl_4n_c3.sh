...
#PBS -N s444cl_4c3
#PBS -j o
#PBS -o s444cl_4n_c3.joblog

# sidia444_cluster node-scaling rerun, CPU PATH -- one of 24 requests
...
#
# Input: sidia444_cluster.dat with System.Name changed and
# "scf.eigen.lib gpusolver" -> "elpa2".  512 Si, s2p2d1 -> matrix dimension
# 6656, real symmetric, Cluster solver at the gamma point, collinear, 25 SCF
# iterations to 1e-13, energycutoff 200 Ry.
#
# Single-sample references (old binary, Max_Time): 1n 264.4 s / 2n 171.0 s /
# 3n 139.0 s / 4n 143.7 s.
# Utot reference: -2103.47785856333x (last digits move with the rank count).
...
echo "=== Utot (reference: -2103.47785856333x) ==="
grep -E "Utot\." "${CASE}.out" 2>/dev/null | tail -2
echo "=== SCF iterations completed (of 25 max) ==="
grep -cE "SCF=" "$STDOUT" 2>/dev/null
echo "=== peak host memory, this node (GiB); old single-sample peaks 1n 110 / 4n 58 ==="
