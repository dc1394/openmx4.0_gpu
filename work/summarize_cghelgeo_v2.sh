#!/bin/bash
# Aggregate the six cghelgeo v2-binary repetitions (3 GPU + 3 CPU, 2 nodes x 48 = 96 ranks).
#
...
GPU_CASES="cghelgeo_v2_g1 cghelgeo_v2_g2 cghelgeo_v2_g3"
CPU_CASES="cghelgeo_v2_c1 cghelgeo_v2_c2 cghelgeo_v2_c3"
...
echo "--- statistics ---"
echo "$gpu_rows" | stats "GPU (gpusolver+MPS)" gpu 2>/tmp/.cg2_gpu_means
echo
echo "$cpu_rows" | stats "CPU (elpa2)"         cpu 2>/tmp/.cg2_cpu_means
echo
...
' /tmp/.cg2_gpu_means /tmp/.cg2_cpu_means
rm -f /tmp/.cg2_gpu_means /tmp/.cg2_cpu_means
echo
