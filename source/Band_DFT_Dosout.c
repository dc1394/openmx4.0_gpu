...
#pragma omp parallel shared(MP,Spe_Total_CNO,WhatSpecies,pi2,SD,factor,Dos,r_energy,DosGauss_Width,DosGauss_Num_Mesh,Dos_Erange_Tmp,l,kloop,spin,EIGEN,atomnum) private(OMPID,Nthrds,Nprocs,GA_AN,wanA,tnoA,Anum,eg,x,iecenter,iewidth,iemin,iemax,ie,xa,i1)
  ...
  ****************************************************/
  ...
  if (myid==Host_ID){
	...
	fprintf(fp,"%15.10f %15.10f %15.10f\n",rtv[1][1]/BohrR,rtv[1][2]/BohrR,rtv[1][3]/BohrR);
	fprintf(fp,"%15.10f %15.10f %15.10f\n",rtv[2][1]/BohrR,rtv[2][2]/BohrR,rtv[2][3]/BohrR);
	fprintf(fp,"%15.10f %15.10f %15.10f\n",rtv[3][1]/BohrR,rtv[3][2]/BohrR,rtv[3][3]/BohrR);
	...
	for (i1=1; i1<=n; i1++){ 
	  ...
	  for (i=0; i<=knum_i; i++){
		...
		fprintf(fp,"%10.5f ",EIGEN[spin][kloop][i1]*eV2Hartree);
	      }
	      fprintf(fp,"\n");
	    }
	...
	for (i=0; i<=knum_i; i++){
	  ...
	  for (j=0; j<=knum_j; j++){
	    ...
	    for (k=0; k<=knum_k; k++){
	      kk = k%knum_k;
	      kloop = TD2OD[ii][jj][kk];
	      fprintf(fp,"%10.5f ",EIGEN[kloop][i1]*eV2Hartree);
	    }
	    fprintf(fp,"\n");
	  }
	  ...
	  for (l=iemin; l<=iemax; l++){
	    ...
	    for (GA_AN=1; GA_AN<=atomnum; GA_AN++){
		  ...
		  for ( i3=0; i3<COHP_num_pairs; i3++ ){
			...
			/*
			u2 = H2[spin][l][Bnum+j1].r*H2[spin][l][Anum+i1].r;
			v2 = H2[spin][l][Bnum+j1].i*H2[spin][l][Anum+i1].i;
			uv = H2[spin][l][Bnum+j1].r*H2[spin][l][Anum+i1].i;
			vu = H2[spin][l][Bnum+j1].i*H2[spin][l][Anum+i1].r;
			*/
      ...
      if (fermisurfer_output) {
        for (GA_AN = 1; GA_AN <= atomnum; GA_AN++) {
          ...
          if ((fp = fopen(file_fermi, "w")) != NULL) {
            ...
            fprintf(fp, "%d %d %d\n", knum_i, knum_j, knum_k);
            fprintf(fp, "%d\n", 1);  // 0: Monkhorst-Pack, 1: (i-1)/Ni, 3: (2i-1)/(2Ni) 
            fprintf(fp, "%d\n", iemax_fs[spin] - iemin_fs[spin] + 1);
          ...
          } /* end of if ((fp = fopen(file_fermi, "w")) != NULL) */
