...
/* GPU CuSolver context for Divide_Conquer_LNO (added by H.Kawai, ported from 3.9.9 GPU) */
...
static void DCLNO_CuSolver_Init(void)
{
    ...
    if (ctx->initialized) DCLNO_CuSolver_Destroy();
                          ...
                          double Eele0[2], double Eele1[2])
{
  ...
  if (scf_eigen_lib_flag == CuSOLVER && Size_Total_Matrix >= GPU_CPU_SWITCH_NUM2) {
