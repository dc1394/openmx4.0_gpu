...
/* GPU CuSolver context for Divide_Conquer (added by H.Kawai, ported from 3.9.9 GPU) */
static int DC_GPU_Threshold(void)
{
    ...
    if (!initialized) {
        ...
        if (env != NULL) threshold = atoi(env);
        initialized = 1;
    }
    return threshold;
}
...
static void DC_CuSolver_Destroy(void)
{
    ...
    if (ctx->d_A != NULL)      wait_cudafunc(cudaFree(ctx->d_A));
    if (ctx->d_C != NULL)      wait_cudafunc(cudaFree(ctx->d_C));
    ...
    if (ctx->h_matrix != NULL) free(ctx->h_matrix);
...
static void DC_CuSolver_Init(void)
{
    ...
    if (ctx->initialized) DC_CuSolver_Destroy();
                      ...
                      double Eele0[2], double Eele1[2])
{
  ...
  /* GPU dispatch (added by H.Kawai): assign CUDA/OpenACC device when CuSOLVER is requested.
     The size threshold uses DC_GPU_Threshold() (env: OPENMX_DC_GPU_THRESHOLD). */
  if (scf_eigen_lib_flag == CuSOLVER && Size_Total_Matrix >= GPU_CPU_SWITCH_NUM) {
