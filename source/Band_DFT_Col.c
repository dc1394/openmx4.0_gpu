...
/* GPU CuSolver context for band collinear (added by H.Kawai, ported from 3.9.9 GPU) */
typedef struct {
    int                device_id;
    int                matrix_dim;
    int                scale_dim;
...
static void BandCol_CuSolver_Destroy(void)
{
    ...
    if (ctx->h_scale != NULL)  free(ctx->h_scale);
...
static void BandCol_CuSolver_Init(void)
{
    ...
    if (ctx->cublas != NULL) BandCol_CuSolver_Destroy();
...
#pragma acc host_data use_device(A, B, C)
    {
        ...
        wait_cudafunc(openmx_gemmul8Zgemm(BandCol_cusolver_ctx.cublas, transa, transb, m, n, k, &alpha,
...
void Calculate_ParDM_AK1( int myworld1, int myworld2, int numprocs0,
			  int myid0, int myid1, int myid2, int size_H1,
			  int spin, int kloop, double ChemP, 
			  int *T_k_op, 
			  double *T_KGrids1, double *T_KGrids2, double *T_KGrids3, 
			  double *H1, int *is2, int *ie2, int *MP,
			  double ***EIGEN, dcomplex **EVec1, double *CDM1,
			  double **ReEVec0, double **ImEVec0, 
			  double **ReEVec1, double **ImEVec1,
			  MPI_Comm *MPI_CommWD1, int *Comm_World_StartID1 );

void Calculate_ParDM_NonAK1(int spin, int kloop, int MaxN, int *MP,
			    int *T_k_op, 
			    double *T_KGrids1, double *T_KGrids2, double *T_KGrids3, 
		  	    double ***EIGEN, dcomplex **EVec1, double *ParDM1,
			    double **ReEVec0, double **ImEVec0, 
			    double **ReEVec1, double **ImEVec1);
		    ...
		    MPI_Comm *MPI_CommWD2)
{
  ...
  double *EVec_Snd,*EVec_Rcv,tmparray[3];
  ...
  ***********************************************/
  ...
  if (ParDM_flag==1){
    size_H1 = Get_OneD_HS_Col(0, CntOLP, S1, MP, order_GA, My_NZeros, SP_NZeros, SP_Atoms);
    ParDM1 = (double*)malloc(sizeof(double)*size_H1);
    for (i=0; i<size_H1; i++){ ParDM1[i] = 0.0; }
  }
...
diagonalize1:
  ...
  ****************************************************/
  ...
  if (all_knum==1){
    ...
    /* calculation of ParDM used for CWF */

    if (ParDM_flag==1){

      spin = myworld1;    
      kloop = S_knum;
      Calculate_ParDM_AK1( myworld1, myworld2, numprocs0, 
			   myid0, myid1, myid2, size_H1, spin, kloop, 
			   ChemP, T_k_op, T_KGrids1, T_KGrids2, T_KGrids3, 
			   H1, is2, ie2, MP, EIGEN, EVec1, CDM1, 
			   ReEVec0, ImEVec0, ReEVec1, ImEVec1, MPI_CommWD1, 
			   Comm_World_StartID1 );
    }
  ...
  diagonalize2:
	...
	Hs[i].i = 0.0;
      ...
      ****************************************************/
      ...
      if (ParDM_flag==1){
	Calculate_ParDM_NonAK1(spin,kloop,MaxN,MP,T_k_op,T_KGrids1,T_KGrids2,T_KGrids3,
                               EIGEN,EVec1,ParDM1,ReEVec0,ImEVec0,ReEVec1,ImEVec1);
      }
    ...
    *******************************************************/
    ...
    if (ParDM_flag==1){
      MPI_Allreduce(MPI_IN_PLACE, ParDM1, size_H1, MPI_DOUBLE, MPI_SUM, MPI_CommWD1[myworld1]);
    }
	...
	if (myid0==ID){
	  ...
	  for (i=0; i<tnoA; i++){
	    for (j=0; j<tnoB; j++){
	      CDM[spin][MA_AN][LB_AN][i][j] = H1[k];
              EDM[spin][MA_AN][LB_AN][i][j] = S1[k];
              if (ParDM_flag==1) ParDM[spin][MA_AN][LB_AN][i][j] = ParDM1[k];
    ...
    if (1<numprocs0 && SpinP_switch==1 && ParDM_flag==1){
	...
	if (myid0==IDS){
	  MPI_Isend(ParDM1, size_H1, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &request);
	}

	if (myid0==IDR){
	  MPI_Recv(H1, size_H1, MPI_DOUBLE, IDS, tag, mpi_comm_level1, &stat);
	}
	...
	if (myid0==IDS){
      ...
      MPI_Bcast(H1, size_H1, MPI_DOUBLE, 0, MPI_CommWD1[myworld1]);
	...
	for (LB_AN=0; LB_AN<=FNAN[GA_AN]; LB_AN++){
	  ...
	  for (i=0; i<tnoA; i++){
	    for (j=0; j<tnoB; j++){
	      ...
	      if (1<=MA_AN && MA_AN<=Matomnum){   
		ParDM[spin][MA_AN][LB_AN][i][j] = H1[k];
	      }
	...
	for (i=0; i<tnoA; i++){
	  for (j=0; j<tnoB; j++){
	    ...
	    iDM[0][spin][MA_AN][LB_AN][i][j] = iDM[0][spin][MA_AN][LB_AN][i][j]*dum;
            ...
            if (ParDM_flag==1){
  	      ParDM[spin][MA_AN][LB_AN][i][j] = ParDM[spin][MA_AN][LB_AN][i][j]*dum;
	    }
	  }
	}
      }
  ...
  if (ParDM_flag==1){
	...
	for (LB_AN=0; LB_AN<=FNAN[GA_AN]; LB_AN++){
	  ...
	  Bnum = MP[GB_AN];
          ...
          printf("\nCDM spin=%2d GA_AN=%2d LB_AN=%2d\n",spin,GA_AN,LB_AN);
	  for (i=0; i<tnoA; i++){
	    for (j=0; j<tnoB; j++){
	      printf("%6.3f ",CDM[spin][MA_AN][LB_AN][i][j]);
	    }
            printf("\n");
	  }

          printf("ParDM spin=%2d GA_AN=%2d LB_AN=%2d\n",spin,GA_AN,LB_AN);
	  for (i=0; i<tnoA; i++){
	    for (j=0; j<tnoB; j++){
	      printf("%6.3f ",ParDM[spin][MA_AN][LB_AN][i][j]);
	    }
            printf("\n");
	  }
...
void Calculate_ParDM_AK1( int myworld1, int myworld2, int numprocs0,
			  ...
			  MPI_Comm *MPI_CommWD1, int *Comm_World_StartID1 ) 
{
  int n,p,i,j,i1,j1,k,GA_AN,wanA,tnoA,Anum;
  int LB_AN,GB_AN,Rn,wanB,tnoB,Bnum;  
  int kmin,kmax,l1,l2,l3,MA_AN,ID;
  double kw,k1,k2,k3,d1,d2,si,co,x,kRn;
  double e,b0,b1,e0,e1,ReA,ImA,*windowF;
  int tag=999,IDS,IDR;
  MPI_Status stat;
  MPI_Request request;
  ...
  for (i=0; i<size_H1; i++){ CDM1[i] = 0.0; }
  ...
  /* store DM1 to a proper place in ParDM and EDM */
  ...
  for (GA_AN=1; GA_AN<=atomnum; GA_AN++){
	...
	for (i=0; i<tnoA; i++){
	  for (j=0; j<tnoB; j++){
	    ...
	    ParDM[spin][MA_AN][LB_AN][i][j] = CDM1[p];
...
void Calculate_ParDM_NonAK1(int spin, int kloop, int MaxN, int *MP,
			    ...
			    double **ReEVec1, double **ImEVec1)
{
  int p,i,j,k,GA_AN,wanA,tnoA,wanB,tnoB,Anum,Bnum,GB_AN;
  int kmin,kmax,l1,l2,l3,Rn,i1,j1,n,LB_AN;
  double e0,e1,e,b0,b1;
  double kw,co,si,kRn,d1,d2,*windowF;
  double ReA,ImA,k1,k2,k3,eig,x;
